...
int main()
{
	...
	if (mySocket != INVALID_SOCKET)
	{
		result = closesocket(mySocket); //uzavreni socketu
		if (result != 0)
		{
...
void datach(const sockaddr* connectionAddress) {
		...
		connect(dataSocket, connectionAddress, sizeof(sockaddr_in));
	...
	while (1)
	{
		...
		if (getStatus(mySocket, STATUS_READ) == 1)
		{
			...
			do
			{
				...
				case 1:
					send(mySocket, okii1, sizeof(okii1), 0);
					printf("POSLAN ping\n");
					break;
				case 2:
				{
					...
					result = send(mySocket, buffer, jsonsLen + 4, 0);
				...
				case 3:
					...
					while ((result = recv(mySocket, buffer + totalBytesReceived, bufferSize - totalBytesReceived, 0)) > 0) {
					...
					send(mySocket, okii3, sizeof(okii3), 0);
		...
		else if (getStatus(dataSocket, STATUS_READ) == 1)
		{
			...
			do
			{
				ioctlsocket(dataSocket, FIONREAD, &howMuchInBufferdata);
				...
				if (dataCH) {
					...
					resultd = recvfrom(dataSocket, recvbufDv, MAX_PACKET_SIZE, 0, (struct sockaddr*)&si_other, &slen);
				...
				if (dataCH) {
					...
					result = send(dataSocket, buffer, jsonsLen + 4, 0);
		...
		else if (getStatus(endSock, STATUS_READ) == 1)
		{
			...
			do
			{
				...
				ioctlsocket(dataSocket, FIONREAD, &howMuchInBufferend);
				...
				if (EndCH) {
					...
					//print out the recived buffer to the console 
					printf("END POSILA \"");
			...
			printf("\" %d bytes on data%c", numBytesReadend,
				((numBytesReadend != 0) ? '\n' : '\r'));
		}
